...
def estimate_project_cost(project: ProductProject, module_count: int | None = None) -> CostEstimate:
    ...
    text_calls = 1 + int(project.options.back_translation_check)
    if (project.options.ocr_check or project.options.image_compliance_check) and project.options.generate_ai_images:
        text_calls += count
    input_tokens = max(800, 700 + count * 320) * text_calls if project.options.optimize_copy_with_ai else 0
    output_tokens = max(500, count * 190) * text_calls if project.options.optimize_copy_with_ai else 0
